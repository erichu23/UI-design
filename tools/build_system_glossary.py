...
TERM_SECTIONS = [
    ("01", "平台、项目与工作簿", "定义系统信息架构和用户在平台中的工作边界。", [
        ...
        ("SYS-005", "数据上传及管理", "dataManagement", "功能模块", "负责原始数据上传和数据质量校验。", "包含数据上传、数据校验两个页签。", "数据上传、数据校验（作为模块总称）"),
        ("SYS-006", "资金流水分析", "fundFlowAnalysis", "功能模块", "基于银行流水和相关主档开展分析范围、账户、经营、特殊交易及明细分析。", "数据源仍称“银行流水”；模块统一称“资金流水分析”。", "银行流水分析"),
    ...
    ("06", "数据校验、特殊交易与报告", "统一质量问题、规则命中、风险线索和成果导出术语。", [
        ...
        ("CHK-002", "分析范围清单核对", "analysisScopeReconciliation", "范围校验", "对比外部工商数据与被审计单位主档，识别公司范围遗漏。", "两方共同包含才视为核对一致；缺失时可补充至主档。", "公司主档遗漏检查"),
