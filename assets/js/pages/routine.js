/* =============== 特定工商查询 Routine 区域 =============== */
    (function(){
      ...
      function getPageItems(current, pages){
        const items = [1, pages, current - 1, current, current + 1]
      ...
      window.renderPagination = window.renderPagination || function(eleId, total, current, handler){
        ...
        let html = `
          <button class="btn" ${current===1?'disabled':''} onclick="${handler}(${current-1})">上一页</button>
        `;

        pageItems.forEach((page, idx)=>{
          if(idx > 0 && page - pageItems[idx-1] > 1){
            html += '<span class="pager-ellipsis">...</span>';
          }
          html += `<button class="btn ${page===current?'btn-primary':''}" onclick="${handler}(${page})">${page}</button>`;
        });

        html += `
          <button class="btn" ${current===pages?'disabled':''} onclick="${handler}(${current+1})">下一页</button>
        `;
        host.innerHTML = html;
      };
      ...
      const ROUTINE_CONFIG = {
        '2.1': {
          title:'经营时间汇总指标',
          scatterTitle:'经营时间分布表',
          xName:'经营天数',
          labels: ROUTINE_DEFAULTS['2.1'].labels,
          breaks: ROUTINE_DEFAULTS['2.1'].years.map(x=>x*365),
          metric: r => r.foundedDays
        },
        '2.2': {
          title:'社保缴纳人数汇总指标',
          scatterTitle:'社保缴纳人数分布表',
          xName:'缴纳人数',
          labels: ROUTINE_DEFAULTS['2.2'].labels,
          breaks: ROUTINE_DEFAULTS['2.2'].breaks,
          metric: r => r.employees
        },
        '2.4': {
          title:'注册资本与交易规模匹配情况',
          scatterTitle:'交易总额 / 注册资本分布表',
          xName:'交易总额/注册资本',
          labels: ROUTINE_DEFAULTS['2.4'].labels,
          breaks: ROUTINE_DEFAULTS['2.4'].breaks,
          metric: r => {
            const capital = Math.max(Number(r.capital)||1, 1);
            return (Number(r.total)||0) / capital;
          }
        },
        '2.5': {
          title:'交易规模与行业状况匹配情况',
          scatterTitle:'行业交易规模分布表',
          xName:'行业交易规模分位',
          labels:['明显偏低','正常偏低','正常偏高','明显偏高'],
          breaks:[0.25,0.5,0.85],
          metric: r => Math.min(1, ((Number(r.total)||0) % 40000) / 40000)
        },
        '2.6': {
          title:'经营范围与交易性质匹配情况',
          scatterTitle:'经营范围匹配度分布表',
          xName:'不匹配评分',
          labels:['低风险','关注','较高风险','高风险'],
          breaks:[25,50,75],
          metric: r => ((Number(r.id)||0) * 17 + (Number(r.txCnt)||0)) % 100
        },
        '2.8': {
          title:'工商登记信息相同或相近情况',
          scatterTitle:'工商信息相似度分布表',
          xName:'相似度评分',
          labels:['低相似','一般相似','较高相似','高度相似'],
          breaks:[30,60,85],
          metric: r => ((Number(r.id)||0) * 23 + String(r.name||'').length * 7) % 100
        },
        '2.9': {
          title:'客户供应商重合及同控情况',
          scatterTitle:'客户/供应商双向交易分布表',
          xName:'双向交易占比',
          labels:['单向交易','轻度双向','明显双向','高度双向'],
          breaks:[0.1,0.35,0.65],
          metric: r => {
            const inflow = Number(r.inflow)||0;
            const outflow = Number(r.outflow)||0;
            return Math.min(inflow, outflow) / Math.max(inflow + outflow, 1);
          }
        }
      };
      ...
      const ROUTINE_COLORS = ['#ef4444','#f59e0b','#2563eb','#2563eb'];
      ...
      /* ---------- 通用分组：按当前 Routine 的 breaks 分 4 段 ---------- */
      function groupByBreaks(list){
        const b = routineState.breaks || [];
        const groups = [[],[],[],[]];
        list.forEach(r=>{
          const x = getRoutineMetric(r);
          let idx = 3;
          if (b && b.length===3){
      ...
      function getRoutineMetric(row){
        const conf = ROUTINE_CONFIG[routineState.code];
        return conf && typeof conf.metric === 'function' ? Number(conf.metric(row)) || 0 : 0;
      }

      function formatMetricValue(v){
        if (routineState.code === '2.1') return `${Math.round(v)} 天`;
        if (routineState.code === '2.2') return `${Math.round(v)} 人`;
        if (['2.4','2.5','2.9'].includes(routineState.code)) return `${Number(v || 0).toFixed(2)}`;
        return `${Math.round(v)}`;
      }
      ...
      function renderScatterAndBox(){
        ...
        let xName = ROUTINE_CONFIG[code]?.xName || 'X';
        ...
        if (code === '2.3'){
          ...
          // 其他 Routine：按配置指标分 4 组，X=指标，Y=交易金额
          groups = groupByBreaks(list);
          series = groups.map((g,i)=>({
            name: routineState.labels[i] || ('区间'+(i+1)),
            type:'scatter',
            data: g.map(r=>{
              const xVal = getRoutineMetric(r);
              return {
                name: r.name,
                value: [xVal, r.total],
                symbolSize: Math.max(10, Math.sqrt(Math.abs(r.total||0))/40 + 8),
                itemStyle:{ color: ROUTINE_COLORS[i] },
                extra: r
              };
            }),
            emphasis:{ focus:'series', scale:1.15 }
        ...
        window.ez('chart-scatter',{
          ...
          tooltip: {
            ...
            formatter: p => {
              ...
              const xVal = code === "2.3"
                ? d.txCnt + " 笔"
                : formatMetricValue(getRoutineMetric(d));
        ...
        if (!scatterResizeBound) {
          window.addEventListener('resize', ()=>{
            const el = document.getElementById('chart-scatter');
            const inst = el && echarts.getInstanceByDom(el);
            inst && inst.resize();
          });
          scatterResizeBound = true;
        }
      ...
      function renderRoutineTable(){
        ...
        const conf = ROUTINE_CONFIG[routineState.code];
        const metricLabel = routineState.code === '2.3' ? '主体类型' : (conf?.xName || '风险指标');
        ...
        tbody.innerHTML = pageItems.map(d=>`
          <tr>
            <td>${d.name||'-'}</td>
            <td style="color:var(--flow-in)">${fmtWan(d.inflow)}</td>
            <td style="color:var(--flow-out)">${fmtWan(d.outflow)}</td>
            <td><b>${fmtWan(d.total)}</b></td>
            <td>${routineState.code === '2.3' ? (d.ctype || '-') : formatMetricValue(getRoutineMetric(d))}</td>
      ...
      function renderRoutine(code){
        ...
        if (code==='2.3'){
          ...
          const conf = ROUTINE_CONFIG[code] || ROUTINE_CONFIG['2.1'];
          routineState.breaks = conf.breaks.slice();
          routineState.labels = conf.labels.slice();
          const t1 = document.getElementById('rTitle'); if(t1) t1.textContent = conf.title;
          const st = document.getElementById('scatterTitle'); if(st) st.textContent = conf.scatterTitle;
        }
    ...
    // 初始进入
    window.renderRoutine && window.renderRoutine("2.1");
