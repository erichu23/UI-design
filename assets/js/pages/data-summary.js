(function(){
  ...
  function crossYearOptions(rows=baseRows()){ return [...new Set(rows.map(r=>r.month.slice(0,4)))].sort((a,b)=>b.localeCompare(a)); }
  function syncCrossYears(rows=baseRows()){
    const opts = crossYearOptions(rows);
    const kept = state.crossYears.filter(y=>opts.includes(y)).slice(0,3);
    state.crossYears = kept.length >= 2 ? kept : opts.slice(0,2);
    return opts;
  }
  ...
  function renderKpis(rows){ const t=sum(rows); $("ds3KpiGrid").innerHTML=[
    ["银行流入合计",t.inflow,"inflow"],["销项税价合计",t.salesVat,"output"],[diffTitle("in"),t.inflow-t.salesVat,"compare"],["覆盖月份数",t.months.size,"count","个月"],
    ["银行流出合计",t.outflow,"outflow"],["进项税价合计",t.purchaseVat,"input"],[diffTitle("out"),t.outflow-t.purchaseVat,"compare"],["当前对手方数量",t.cps.size,"count","家"]
  ...
  function renderCharts(rows){
    if(state.trendMode==="crossYear"){
      ...
      renderCrossYearChart("ds3InChart",["银行流入","销项税价合计"],["#2f6fed","#0f9f8a"],"inflow","salesVat");
      renderCrossYearChart("ds3OutChart",["银行流出","进项税价合计"],["#d9466f","#d97706"],"outflow","purchaseVat");
      return;
    }
    renderCrossYearToolbar();
  ...
  function renderCrossYearChart(id,legend,colors,aKey,bKey){
    const el=$(id); if(!el)return;
    ...
    const yearMaps=crossYearMonthlyData();
    const selectedYears=state.crossYears.slice(0,3);
    const x=months.map(m=>`${m}月`);
    const isLine=state.chartType==="line", diffLabel=diffInfo(aKey==="inflow"?"in":"out").label;
