(function(){
  ...
  function auditPagerRange(page, pages){
    ...
    if(current<=4){start=2;end=6;} if(current>=total-3){start=total-5;end=total-1;}
    if(start>2)out.push("ellipsis-start"); for(let i=start;i<=end;i++)out.push(i); if(end<total-1)out.push("ellipsis-end"); out.push(total); return out;
  }
  function renderAuditPager(el,{total,page,pageSize,onPage,onPageSize}){
    if(!el)return; total=Number(total)||0; pageSize=Number(pageSize)||20; const pages=Math.max(1,Math.ceil(total/pageSize)); page=Math.max(1,Math.min(Number(page)||1,pages));
    el.classList.add("audit-pager");
    el.innerHTML=`<span class="audit-pager-total">共 ${total.toLocaleString("zh-CN")} 条</span><button class="audit-pager-btn audit-pager-arrow" type="button" data-page="prev" ${page<=1?"disabled":""}>‹</button>${auditPagerRange(page,pages).map(item=>typeof item==="string"?`<span class="audit-pager-ellipsis">...</span>`:`<button class="audit-pager-btn ${item===page?"is-active":""}" type="button" data-page="${item}">${item}</button>`).join("")}<button class="audit-pager-btn audit-pager-arrow" type="button" data-page="next" ${page>=pages?"disabled":""}>›</button><label class="audit-pager-size"><select data-page-size>${[10,20,30,40,50].map(n=>`<option value="${n}" ${n===pageSize?"selected":""}>${n} 条/页</option>`).join("")}</select></label><span class="audit-pager-jump-label">跳至</span><input class="audit-pager-jump" type="number" min="1" max="${pages}" inputmode="numeric" data-page-jump><span class="audit-pager-jump-label">页</span>`;
    el.querySelectorAll("[data-page]").forEach(btn=>btn.onclick=()=>{let next=page;if(btn.dataset.page==="prev")next=page-1;else if(btn.dataset.page==="next")next=page+1;else next=Number(btn.dataset.page)||page;onPage?.(Math.max(1,Math.min(next,pages)));});
    el.querySelector("[data-page-size]")?.addEventListener("change",e=>onPageSize?.(Number(e.target.value)||20));
    const jump=el.querySelector("[data-page-jump]"); const go=()=>{if(!jump.value)return;onPage?.(Math.max(1,Math.min(Number(jump.value)||page,pages)));};
    jump?.addEventListener("change",go); jump?.addEventListener("keydown",e=>{if(e.key==="Enter")go();});
  }
  const fmt = n => Number(n || 0).toLocaleString("zh-CN", { maximumFractionDigits: 1 });
  const fmtK = n => window.AuditUnit?.ready ? window.AuditUnit.formatFromK(n) : `${fmt(n)}K`;
  const chartAmount = n => window.AuditUnit?.ready ? window.AuditUnit.convert(n,"k") : Number(n||0);
  const chartAmountList = values => values.map(chartAmount);
  const chartUnitLabel = () => window.AuditUnit?.ready ? window.AuditUnit.label : "千元";
  const chartAxisTitle = () => `金额（${chartUnitLabel()}）`;
  const fmtTableAmount = n => fmt(n);
  const fmtMetric = (value, unit) => unit ? fmt(value) : fmtK(value);
  ...
  window.addEventListener("audit-unit-ready",()=>setTimeout(refreshVisibleCharts,0),{once:true});
  ...
  function syncCrossYears(rows=baseRows()){
    ...
    state.crossYears = kept.length >= 1 ? kept : opts.slice(-3);
    return opts;
  }
  ...
  function initFilters(){
    ...
    if($("ds3Type"))$("ds3Type").innerHTML = ["全部","客户","供应商","其他"].map(x=>`<option>${x}</option>`).join("");
    syncFilterControls();
    refreshAccounts();
    renderMonthToolbar();
  }
  ...
  function renderCrossYearToolbar(){
    ...
    if(opts.length < 1){
      host.innerHTML=`<div class="ds3-crossyear-note">暂无可用于同月对比的年份数据</div>`;
      return;
    }
    host.innerHTML=`<div class="ds3-crossyear-note">默认展示最近三年，可勾选任意 1-3 个年份；颜色越深表示年份越近</div><div class="ds3-crossyear-picker" id="ds3CrossYearPicker">${opts.map(y=>{const checked=state.crossYears.includes(y),disabled=(!checked&&state.crossYears.length>=3)||(checked&&state.crossYears.length<=1);return `<label class="ds3-year-chip ${checked?"is-active":""} ${disabled?"is-disabled":""}"><input type="checkbox" value="${y}" ${checked?"checked":""} ${disabled?"disabled":""}/> ${y}年</label>`;}).join("")}</div>`;
  ...
  ].map(([k,v,c,u])=>`<div class="ds3-vat-kpi is-${c}"><span>${k}</span><b><strong>${fmtMetric(v,u)}</strong><em>${unitText(u)}</em></b></div>`).join(""); }
  ...
  function renderCharts(rows){
    if(state.trendMode==="crossYear"){
      ...
      const opts = syncCrossYears();
      if(opts.length < 1){
        showChartEmpty("ds3InChart","暂无可用于同月对比的年份数据");
        showChartEmpty("ds3OutChart","暂无可用于同月对比的年份数据");
        return;
      }
  ...
  function renderChart(id,side,m,legend,colors,aKey,bKey,chartType){ const el=$(id); if(!el)return; el.classList.remove("is-crossyear-layered"); clearCrossYearSummary(el); if(!window.echarts){el.innerHTML='<div class="ds3-chart-loading">图表加载中</div>';ensureEcharts();return;} const chart=getChartInstance(id,el); const x=m.map(r=>state.year==="all"?r.month.replace("-","年")+"月":r.month.slice(5)+"月"); const a=m.map(r=>r[aKey]); const b=m.map(r=>r[bKey]); const diff=a.map((v,i)=>+(v-b[i]).toFixed(1)); const chartA=chartAmountList(a),chartB=chartAmountList(b),chartDiff=chartAmountList(diff); const isLine=(chartType||state.chartType)==="line", diffLabel=diffInfo(side).label, isMainMonthly=id==="ds3InChart"||id==="ds3OutChart", gridTop=isMainMonthly?28:38, gridBottom=isMainMonthly?22:58, zoomBottom=isMainMonthly?-1:12, zoomHeight=isMainMonthly?10:18; chart.setOption({color:colors,tooltip:{trigger:"axis",confine:true,formatter:params=>{const idx=params[0]?.dataIndex||0;return `<div class="ds3-tip-card"><b>${params[0]?.axisValue||""}</b><div>${legend[0]}：${fmtK(a[idx])}</div><div>${legend[1]}：${fmtK(b[idx])}</div><div>${diffLabel}：${fmtK(diff[idx])}</div><div>账期口径：${periodName()}</div></div>`;}},legend:{top:0,itemHeight:8,itemWidth:12,textStyle:{fontSize:11},data:legend},grid:{left:50,right:14,top:gridTop,bottom:gridBottom},dataZoom:[{type:"slider",height:zoomHeight,bottom:zoomBottom,start:0,end:x.length>12?42:100,brushSelect:false,showDetail:false},{type:"inside"}],xAxis:{type:"category",data:x},yAxis:{type:"value",name:chartAxisTitle()},series:isLine?[{name:legend[0],type:"line",smooth:true,showSymbol:true,symbolSize:5,lineStyle:{width:2.4,color:colors[0]},itemStyle:{color:colors[0]},data:chartA},{name:legend[1],type:"line",smooth:true,showSymbol:true,symbolSize:5,lineStyle:{width:2.4,color:colors[1]},itemStyle:{color:colors[1]},data:chartB}]:[{name:legend[0],type:"bar",barMaxWidth:22,data:chartA},{name:legend[1],type:"bar",barMaxWidth:22,data:chartB},{name:diffLabel,type:"line",smooth:true,itemStyle:{color:"#7c3aed"},lineStyle:{color:"#7c3aed"},data:chartDiff}]}, true); chart.resize(); chart.off("click"); chart.on("click",p=>{ if(state.year==="all")return; const mm=String(p.name).slice(0,2); state.selectedMonths=[`${state.year}-${mm}`]; state.page=1; renderMonthSelect(); renderAll();}); }
  ...
  function median(values){const nums=values.map(v=>+v||0).sort((a,b)=>a-b);if(!nums.length)return 0;const mid=Math.floor(nums.length/2);return nums.length%2?nums[mid]:(nums[mid-1]+nums[mid])/2;}
  function signedFmt(value){const n=+(value||0);return `${n>0?"+":""}${fmtK(n)}`;}
  function clearCrossYearSummary(el){el?.querySelector?.(".ds3-crossyear-summary")?.remove();}
  function crossYearStats(yearMaps, selectedYears, aKey, bKey){
    const aValues=[], bValues=[];
    let highest={value:-Infinity,month:"--"};
    selectedYears.forEach(year=>months.forEach(month=>{
      const r=yearMaps.get(year)?.get(month)||{};
      const a=+(r[aKey]||0);
      const b=+(r[bKey]||0);
      aValues.push(a);
      bValues.push(b);
      if(a>highest.value)highest={value:a,month};
    }));
    return {aMedian:median(aValues), bMedian:median(bValues), highestMonth:highest.month==="--"?"--":`${highest.month}月`};
  }
  function renderCrossYearSummary(el, legend, selectedYears, stats){
    clearCrossYearSummary(el);
    const node=document.createElement("div");
    node.className="ds3-crossyear-summary";
    node.innerHTML=`<span class="ds3-crossyear-summary-years">${selectedYears.join(" / ")}｜深色为较近年份</span><span>${legend[0].replace("银行","")}中位数：<b>${fmtK(stats.aMedian)}</b></span><span>${legend[1].replace("税价合计","")}中位数：<b>${fmtK(stats.bMedian)}</b></span><span>最高月份：<b>${stats.highestMonth}</b></span>`;
    el.appendChild(node);
  }
  function medianMarkLine(label,value,color){
    return {
      symbol:"none",
      silent:false,
      tooltip:{formatter:()=>`${label}中位数：${fmtK(value)}`},
      label:{
        show:true,
        formatter:`${label}中位数 ${fmtK(value)}`,
        position:"end",
        distance:4,
        color:"#475569",
        fontSize:10,
      ...
      data:[{name:`${label}中位数`,yAxis:+chartAmount(value).toFixed(3)}]
    };
  }
  function renderLayeredCrossYearChart(id,legend,aKey,bKey,side,rowsArg=baseRows(),selectedYearsArg=state.crossYears){
    ...
    const stats=crossYearStats(yearMaps, selectedYears, aKey, bKey);
    const makeData=(year,key)=>months.map(m=>{
      const r=yearMaps.get(year)?.get(m)||{};
      const a=+(r[aKey]||0).toFixed(1);
      const b=+(r[bKey]||0).toFixed(1);
      return {value:chartAmount(key===aKey?a:b),year,monthNo:m,a,b,diff:+(a-b).toFixed(1)};
    });
    ...
    chart.setOption({
      ...
      tooltip:{trigger:"item",confine:true,formatter:p=>{const r=p.data||{};const medianRows=showCrossYearAssist?`<div>较${legend[0].replace("银行","")}中位数：${signedFmt((r.a||0)-stats.aMedian)}</div><div>较${legend[1].replace("税价合计","")}中位数：${signedFmt((r.b||0)-stats.bMedian)}</div>`:"";return `<div class="ds3-tip-card"><b>${r.year}年${r.monthNo}月</b><div>${legend[0]}：${fmtK(r.a)}</div><div>${legend[1]}：${fmtK(r.b)}</div><div>${diffLabel}：${fmtK(r.diff)}</div>${medianRows}<div>账期口径：${periodName()}</div></div>`;}},
      title:[
      ...
      graphic:[{type:"text",right:22,top:30,silent:true,style:{text:`单位：${chartUnitLabel()}`,fill:"#94a3b8",font:"10px sans-serif"}}],
      legend:{show:false},
      grid:[
      ...
      dataZoom:[
        {type:"slider",xAxisIndex:[0,1],height:14,bottom:isDetailChart?28:10,left:74,right:34,start:0,end:100,brushSelect:false,showDetail:false},
      ...
      yAxis:[
        {type:"value",name:"",gridIndex:0,splitNumber:3,axisLabel:{color:"#64748b",fontSize:10,margin:8,hideOverlap:true},axisLine:{show:false},axisTick:{show:false},splitLine:{lineStyle:{color:"#eef2f7"}}},
        {type:"value",name:"",gridIndex:1,inverse:true,splitNumber:3,axisLabel:{color:"#64748b",fontSize:10,margin:8,hideOverlap:true,showMinLabel:false},axisLine:{show:false},axisTick:{show:false},splitLine:{lineStyle:{color:"#eef2f7"}}}
      ],
      series
    }, true);
    if(showCrossYearAssist)renderCrossYearSummary(el, legend, selectedYears, stats);
    else clearCrossYearSummary(el);
    chart.resize();
    chart.off("click");
  }
  function renderCrossYearChart(id,legend,colors,aKey,bKey,rowsArg=baseRows(),selectedYearsArg=state.crossYears,chartType=state.chartType){
    ...
    const series=selectedYears.flatMap(y=>[
      {name:legend[0],type:isLine?"line":"bar",smooth:isLine,showSymbol:isLine,barMaxWidth:10,itemStyle:{color:colors[0]},lineStyle:{width:2,color:colors[0]},data:months.map(m=>{const r=yearMaps.get(y)?.get(m)||{};const a=+(r[aKey]||0).toFixed(1),b=+(r[bKey]||0).toFixed(1);return {value:chartAmount(a),year:y,monthNo:m,a,b,diff:+(a-b).toFixed(1)};})},
      {name:legend[1],type:isLine?"line":"bar",smooth:isLine,showSymbol:isLine,barMaxWidth:10,itemStyle:{color:colors[1]},lineStyle:{width:2,color:colors[1]},data:months.map(m=>{const r=yearMaps.get(y)?.get(m)||{};const a=+(r[aKey]||0).toFixed(1),b=+(r[bKey]||0).toFixed(1);return {value:chartAmount(b),year:y,monthNo:m,a,b,diff:+(a-b).toFixed(1)};})}
    ]);
    chart.setOption({color:colors,tooltip:{trigger:"axis",axisPointer:{type:isLine?"line":"shadow"},confine:true,formatter:params=>{const first=params[0]?.data||{},monthNo=first.monthNo||String(params[0]?.axisValue||"").slice(0,2);const rows=selectedYears.map(y=>{const row=params.find(p=>p.data?.year===y)?.data||{};return `<div class="ds3-tip-year"><b>${y}年</b><span>${legend[0]}：${fmtK(row.a)}</span><span>${legend[1]}：${fmtK(row.b)}</span><span>${diffLabel}：${fmtK(row.diff)}</span></div>`;}).join("");return `<div class="ds3-tip-card ds3-tip-card-wide"><b>${monthNo}月</b>${rows}<div>账期口径：${periodName()}</div></div>`;}},legend:{top:0,data:legend},grid:{left:56,right:18,top:44,bottom:58},dataZoom:[{type:"slider",height:18,bottom:12,start:0,end:100,brushSelect:false},{type:"inside"}],xAxis:{type:"category",data:x,axisTick:{alignWithLabel:true}},yAxis:{type:"value",name:chartAxisTitle()},series}, true);
    chart.resize();
    chart.off("click");
  }
...
function renderComposition(rows){ const rank=state.compositionRank||"inflow"; const limit=Math.max(1,Math.min(100,+state.compositionLimit||20)); if($("ds3CompositionLimit"))$("ds3CompositionLimit").value=limit; document.querySelectorAll("#ds3CompositionMetric [data-composition-rank]").forEach(b=>b.classList.toggle("is-active",b.dataset.compositionRank===rank)); const list=byCp(rows).map(c=>({...c,name:c.counterparty})).sort((a,b)=>(b[rank]||0)-(a[rank]||0)).slice(0,limit); const max=Math.max(...list.flatMap(x=>[x.inflow,x.salesVat,x.outflow,x.purchaseVat]),1); const bar=(row,field,label,cls,side)=>`<div class="ds3-centipede-bar is-${side}"><span class="ds3-centipede-label">${label}</span><div class="ds3-centipede-track"><i class="is-${cls}" style="width:${Math.max(3,(row[field]||0)/max*100)}%"></i></div><b>${fmtK(row[field])}</b></div>`; $("ds3Composition").innerHTML=`<div class="ds3-centipede-head"><span>流入 / 销项</span><span>对手方</span><span>流出 / 进项</span></div><div class="ds3-centipede-list">${list.map((row,i)=>`<div class="ds3-centipede-row" data-cp="${row.name}"><div class="ds3-centipede-side">${bar(row,"inflow","流入","inflow","left")}${bar(row,"salesVat","销项","output","left")}</div><div class="ds3-centipede-name"><em>${String(i+1).padStart(2,"0")}</em><strong>${row.name}</strong>${cpShift(row.name)!==null?`<span>${shiftLabel(cpShift(row.name))}</span>`:""}</div><div class="ds3-centipede-side">${bar(row,"outflow","流出","outflow","right")}${bar(row,"purchaseVat","进项","input","right")}</div></div>`).join("")}</div>`; $("ds3Composition").querySelectorAll(".ds3-centipede-row").forEach(el=>el.onclick=()=>{state.selectedCounterparty=el.dataset.cp; renderAll(); openDetail(el.dataset.cp);}); }
  ...
  function diffSubHeadCells(sep=false){return `<th class="audit-period-metric${sep?' ds3-col-sep':''}">流入金额</th><th class="audit-period-metric">销项税价合计</th><th class="audit-period-metric">${diffTitle("in")}</th><th class="audit-period-metric">流出金额</th><th class="audit-period-metric">进项税价合计</th><th class="audit-period-metric">${diffTitle("out")}</th>`;}
  function yearRangeTip(year){return `${year}-01-01～${year}-12-31`;}
  function yearGroupTitle(year){return `${year}${infoIcon(`财年期间：${yearRangeTip(year)}`)}`;}
  ...
  function groupCells(t,sep=""){return `<td class="is-num audit-period-metric ds3-num-inflow ${sep}">${fmtTableAmount(t.inflow)}</td><td class="is-num audit-period-metric ds3-num-output">${fmtTableAmount(t.salesVat)}</td><td class="is-num audit-period-metric">${fmtTableAmount(t.inflow-t.salesVat)}</td><td class="is-num audit-period-metric ds3-num-outflow">${fmtTableAmount(t.outflow)}</td><td class="is-num audit-period-metric ds3-num-input">${fmtTableAmount(t.purchaseVat)}</td><td class="is-num audit-period-metric">${fmtTableAmount(t.outflow-t.purchaseVat)}</td>`;}
  ...
  function typeCell(value){const text=String(value??"");const cls=text==="客户"?" is-customer":text==="供应商"?" is-supplier":"";return `<td class="ds3-type-cell${cls}" title="${text.replace(/"/g,"&quot;")}">${text}</td>`;}
  ...
  function renderPager(total){renderAuditPager($("ds3Pagination"),{total,page:state.page,pageSize:state.pageSize,onPage(next){state.page=next;renderAll();},onPageSize(size){state.pageSize=size;state.page=1;renderAll();}});}
  ...
  function renderInvoiceFundingChart(id,titleA,titleB,rows,colors){
    const el=$(id);if(!el)return;clearCrossYearSummary(el);el.classList.remove("is-crossyear-layered");if(!window.echarts){el.innerHTML='<div class="ds3-chart-loading">图表加载中</div>';ensureEcharts();return;}const chart=getChartInstance(id,el),x=rows.map(r=>state.year==="all"?r.month.replace("-","年")+"月":r.month.slice(5)+"月"),total=rows.map(r=>+r.total.toFixed(1)),matched=rows.map(r=>+r.matched.toFixed(1)),unmatched=rows.map(r=>+r.unmatched.toFixed(1));chart.setOption({color:colors,tooltip:{trigger:"axis",confine:true,formatter:p=>{const i=p[0]?.dataIndex||0;return `<div class="ds3-tip-card"><b>${p[0]?.axisValue||""}</b><div>${titleA}：${fmtK(total[i])}</div><div>${titleB}：${fmtK(matched[i])}</div><div>未对应金额：${fmtK(unmatched[i])}</div></div>`;}},legend:{top:0,itemHeight:8,itemWidth:12,textStyle:{fontSize:11},data:[titleA,titleB,"未对应金额"]},grid:{left:54,right:18,top:32,bottom:24},dataZoom:[{type:"slider",height:10,bottom:0,start:0,end:x.length>12?42:100,brushSelect:false,showDetail:false},{type:"inside"}],xAxis:{type:"category",data:x},yAxis:{type:"value",name:chartUnitLabel()},series:[{name:titleA,type:"bar",barMaxWidth:20,data:chartAmountList(total)},{name:titleB,type:"bar",barMaxWidth:20,data:chartAmountList(matched)},{name:"未对应金额",type:"line",smooth:true,symbolSize:5,lineStyle:{width:2,color:colors[2]},itemStyle:{color:colors[2]},data:chartAmountList(unmatched)}]},true);chart.resize();
  }
  ...
  function renderInvoiceSummary(rows){
    ...
    $("ds3InvoiceSummary").innerHTML=`<div class="ds3-panel"><div class="ds3-panel-title">当前查看摘要</div><div class="ds3-invoice-facts"><span>年份：<b>${state.year==="all"?"全年":state.year+"年"}</b></span><span>月份：<b>${state.selectedMonths.length?state.selectedMonths.map(x=>x.slice(5)+"月").join("、"):"全部月份"}</b></span><span>账期口径：<b>${periodName()}</b></span><span>票据数量：<b>${s.count}张</b></span></div></div><div class="ds3-panel"><div class="ds3-panel-title">对应状态分布</div><div class="ds3-invoice-status-list">${status.map(x=>`<div><span>${x.name}</span><b>${x.count}张</b><em>${fmtK(x.amount)}</em></div>`).join("")}</div></div><div class="ds3-panel"><div class="ds3-panel-title">未对应金额较高对手方</div><div class="ds3-invoice-top-list">${top.length?invoiceByCounterparty(rows).filter(x=>x.unmatchedTotal>0).slice(0,3).map(x=>`<div><span>${x.counterparty}</span><b>${fmtK(x.unmatchedTotal)}</b><em>${x.invoiceCount}张</em></div>`).join(""):`<div class="ds3-empty-cell">当前范围内暂无未对应票据</div>`}</div></div>`;
  }
  ...
  function renderInvoiceTable(rows){
    ...
    $("ds3InvoiceTableBody").innerHTML=list.length?list.map(r=>`<tr><td class="ds3-sticky-cp" style="left:0"><span class="ds3-row-main-name">${r.counterparty}</span></td>${typeCell(r.type)}<td class="is-num ds3-num-output">${fmtK(r.salesTotal)}</td><td class="is-num">${fmtK(r.salesMatched)}</td><td class="is-num">${fmtK(r.salesUnmatched)}</td><td class="is-num ds3-num-input">${fmtK(r.purchaseTotal)}</td><td class="is-num">${fmtK(r.purchaseMatched)}</td><td class="is-num">${fmtK(r.purchaseUnmatched)}</td><td class="is-num">${fmt(r.coverageRate)}%</td><td><span class="ds3-invoice-status is-${r.status==="已对应"?"matched":r.status==="部分对应"?"partial":r.status==="长期未匹配"?"long":"open"}">${r.status}</span></td><td class="is-num">${r.invoiceCount}</td><td class="is-num">${r.maxDays||"-"}</td><td class="ds3-sticky-action"><button class="ds3-eye-btn ds3-detail-btn ds3-invoice-detail-btn" data-cp="${r.counterparty}" type="button">查看详情</button></td><td class="ds3-remark-cell ds3-sticky-remark"><input class="ds3-note-input" data-invoice-note="${r.counterparty}" value="${r.remark||""}" placeholder="填写备注"/></td></tr>`).join(""):`<tr><td colspan="14" class="ds3-empty-cell">当前筛选范围暂无对手方票据数据</td></tr>`;
    renderAuditPager($("ds3InvoicePagination"),{total,page:state.invoicePage,pageSize:state.invoicePageSize,onPage(next){state.invoicePage=next;renderInvoiceTable(invoiceFundingRows());},onPageSize(size){state.invoicePageSize=size;state.invoicePage=1;renderInvoiceTable(invoiceFundingRows());}});
  ...
  function bind(){
    ...
    if($("ds3Type"))$("ds3Type").onchange=()=>{state.type=$("ds3Type").value;state.page=1;state.invoicePage=1;renderAll();};
